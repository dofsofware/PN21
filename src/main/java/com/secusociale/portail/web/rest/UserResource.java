...
public class UserResource {
    ...
    public UserResource(UserService userService, UserRepository userRepository, SalarieService salarieService, DocumentUrlService documentUrlService, MailService mailService) {
...
//    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
    ...
    public ResponseEntity<UserDTO> getUser(@PathVariable String login) {
        ...
        Optional<UserDTO> userDTO = userService.getUserWithAuthoritiesByLogin(login)
            .map(UserDTO::new);
        userDTO.ifPresent(dto -> {
                if (!StringUtils.isEmpty(dto.getTypeCompte()) && dto.getTypeCompte().equalsIgnoreCase(Constants.SALARIE))
                    dto.setSalarieDTO(salarieService.findByUserId(dto.getId()).orElse(null));
            }
        );
    ...
    public ResponseEntity<Object> Verification(@Valid @RequestBody LoginVM loginVM) {
        log.debug("REST request to authenticate User");

        try {
            boolean isAuthenticated = userService.checkLogin(loginVM.getUsername(), loginVM.getPassword());
            ...
            if (isAuthenticated) {
                ...
                System.out.println("test invalide pwd");
    ...
    @PutMapping("/users/{id}/cachet")
    public ResponseEntity<UserDTO> updateCachet(
        ...
        if (user.getCachet() == null || user.getCachet().isEmpty()) {
            return ResponseEntity.badRequest().body(null);
        }

        try {
            String path = documentUrlService.uploadMandat(base64ToMultipart(user.getCachet(), "cachet"));
            user.setCachet(path);

            Optional<UserDTO> updatedUser = userService.updateCachet(id, user.getCachet());
